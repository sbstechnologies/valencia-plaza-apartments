...
function getUnitAvailability(plan: FloorPlan): string {
  const unit = units.find((item) => item.name === plan.code);

  return unit?.available ?? "Contact leasing office";
}

function getFloorPlanAlt(plan: FloorPlan): string {
  return `${plan.title} apartment floor plan at ${siteConfig.name} in Fort Worth, Texas`;
}

function getInteriorAlt(plan: FloorPlan): string {
  return `${plan.title} apartment interior at ${siteConfig.name} in Fort Worth, Texas`;
}

function getBedroomCount(value: string): number {
...
function ComparisonDrawer({
  ...
  const rows = [
    ...
    {
      label: "AVAILABLE",
      left: getUnitAvailability(leftPlan),
      right: getUnitAvailability(rightPlan),
    },
  ];
          ...
          <div className="hidden md:grid md:grid-cols-[220px_1fr_1fr] md:gap-8">
            ...
            {[leftPlan, rightPlan].map((plan, planIndex) => (
              <div key={plan.title}>
                ...
                {rows.map((row) => {
                  const otherPlan = planIndex === 0 ? rightPlan : leftPlan;

                  const highlight =
                    row.label === "PRICE"
                      ? cheaperPrice(plan.price, otherPlan.price)
                      : row.label === "SQ FOOTAGE"
                        ? biggerArea(plan.area, otherPlan.area)
                        : false;

                  const value = planIndex === 0 ? row.left : row.right;
            ...
            <div className="px-6 py-8 md:px-8">
              ...
              <div className="mt-7 grid grid-cols-2 gap-4">
                {[
                  ...
                  ["AVAILABLE", getUnitAvailability(plan)],
                ].map(([label, value]) => (
                  <div
                    key={label}
...
export default function Floor() {
  ...
  /*
   * DYNAMIC BEDROOM FILTERS
   *
   * No hard-coded 1 Bedroom / 2 Bedrooms assumptions.
   * Filters are generated from the actual floorPlans data.
   */
  ...
  const availableBedroomCounts = useMemo(() => {
    ...
    floorPlans.forEach((plan) => {
      const bedrooms = getBedroomCount(plan.beds);
  ...
  const bedroomFilters = useMemo(
    () => [
      "All Plans",
      ...availableBedroomCounts.map(
        (count) => `${count} ${count === 1 ? "Bedroom" : "Bedrooms"}`,
      ),
    ],
  ...
  const filteredFloorPlans = useMemo(() => {
    ...
    const bedroomCount = getBedroomCount(selectedBedroom);

    return floorPlans.filter(
      (plan) => getBedroomCount(plan.beds) === bedroomCount,
    );
  }, [selectedBedroom]);
  ...
  const availableUnitCount = units.reduce((total, unit) => {
    const count = Number(unit.available.match(/\d+/)?.[0] ?? 0);

    return total + count;
  }, 0);

  const sqFtValues = floorPlans
    .map((plan) => Number(plan.area.replace(/,/g, "")))
  ...
  const prices = floorPlans
    .map((plan) => Number(plan.price.replace(/[$,\/mo\s]/g, "")))
    .filter(Number.isFinite);
  ...
  const stats = [
    ...
    [String(availableUnitCount), "UNITS AVAILABLE"],
    [
      `${minSqFt.toLocaleString()} - ${maxSqFt.toLocaleString()}`,
      "SQ FT RANGE",
    ],
      ...
      <section className="relative mx-auto min-h-[620px] overflow-hidden bg-[#1f376d] px-6 pb-20 pt-[84px] text-[#F5F2ED] md:px-20 md:py-28 lg:px-40">
        ...
        <div className="relative z-10 max-w-[720px]">
          ...
          <p className="mt-[28px] max-w-[1920px] font-[Plus_Jakarta_Sans] text-[18px] leading-[1.72] tracking-[-0.02em] text-[#b7bfd0] md:text-[20px]">
            Two generously sized two-bedroom condo-style residences featuring
            semi-private building foyers, 10-foot ceilings, cozy fireplaces, and
            in-unit washer/dryer connections designed for elevated living in
            East Fort Worth.
          </p>
