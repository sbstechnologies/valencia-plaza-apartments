...
const pricingText = units
  .map((unit) => `🏠 ${unit.name} · ${unit.label} · ${unit.price}`)
  .join("\n");

const availabilityText = units
  .map(
    (unit) => `🏠 ${unit.name} · ${unit.label} · ${unit.area} · ${unit.price}`,
  )
  .join("\n");

const plansText = units
  .map((unit) => `🏠 ${unit.name} · ${unit.label} · ${unit.area}`)
  .join("\n");
...
const defaultMessages: Message[] = [
  {
    id: 1,
    sender: "bot",
    text: "Hi there! 👋 I'm Lori, your Valencia Plaza Apartment Homes AI assistant. What can I help you with today?",
  },
];
...
export default function LoriChatWidget() {
  ...
  const addMessage = (user: string, bot: string) => {
    setMessages((prev) => [
      ...prev,
      { id: Date.now(), sender: "user", text: user },
      { id: Date.now() + 1, sender: "bot", text: bot },
    ]);
  };
  ...
  const handleSend = () => {
    ...
    if (input.toLowerCase().includes("location")) {
      reply = "Valencia Plaza Apartment Homes is located in FORT WORTH, TX.";
                    ...
                    onClick={() =>
                      handleQuick(
                        ...
                        item.label === "Plans"
                          ? plansText
                          : item.label === "Pets"
                            ? "🐾 Valencia Plaza Apartment Homes is pet friendly — cats and dogs are welcome."
                            : item.label === "Parking"
                              ? "🚗 Resident Parking is available for residents."
                              : "📝 Apply online anytime through the Valencia Plaza Apartment Homes resident portal.",
                      )
                    }
