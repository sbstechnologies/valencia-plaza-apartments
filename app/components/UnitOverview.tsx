...
export default function UnitOverview({
  ...
  applyLink = siteConfig.applyLink,
  planFeatures,
  onScheduleTour,
}: UnitOverviewProps) {
  ...
  /*
   * ------------------------------------------------------------
   * DERIVE BEDROOM TYPES FROM PLAN DATA
   * ------------------------------------------------------------
   */

  const bedroomGroups = useMemo(() => {
    const groups: Record<BedroomType, string[]> = {
      "1bed": [],
      "2bed": [],
    };

    Object.entries(availablePlans).forEach(([planId, plan]) => {
      if (!plan) return;

      const bedroomCount = Number.parseInt(plan.bed, 10);

      if (bedroomCount === 1) {
        groups["1bed"].push(planId);
      }

      if (bedroomCount === 2) {
        groups["2bed"].push(planId);
      }
    });
  ...
  /*
   * ------------------------------------------------------------
   * AVAILABLE BEDROOM TYPES
   * ------------------------------------------------------------
   */

  const availableBedroomTypes = useMemo<BedroomType[]>(
    () =>
      (["1bed", "2bed"] as BedroomType[]).filter(
        (type) => bedroomGroups[type].length > 0,
      ),
    [bedroomGroups],
  );
  ...
  const activeBedroomType = useMemo<BedroomType>(() => {
    if (selectedPlan) {
      const plan = availablePlans[selectedPlan];

      if (plan) {
        return Number.parseInt(plan.bed, 10) === 1 ? "1bed" : "2bed";
      }
    }

    return availableBedroomTypes[0] ?? "2bed";
  }, [selectedPlan, availablePlans, availableBedroomTypes]);

  /*
   * ------------------------------------------------------------
   * PLANS FOR ACTIVE BEDROOM TYPE
   * ------------------------------------------------------------
   */

  const planIds = bedroomGroups[activeBedroomType] ?? [];
  ...
  const safeSelectedPlan =
    selectedPlan && planIds.includes(selectedPlan)
      ? selectedPlan
      : (planIds[0] ?? "");
  ...
  /*
   * ------------------------------------------------------------
   * CURRENT BEDROOM PRICE
   * ------------------------------------------------------------
   */

  const bedroomPricing = useMemo(() => {
    const pricing: Record<BedroomType, string> = {
      "1bed": "Contact Leasing",
      "2bed": "Contact Leasing",
    };

    (["1bed", "2bed"] as BedroomType[]).forEach((type) => {
      const firstPlanId = bedroomGroups[type][0];

      if (firstPlanId && availablePlans[firstPlanId]) {
        pricing[type] = availablePlans[firstPlanId].price;
      }
    });

    return pricing;
  }, [bedroomGroups, availablePlans]);
  ...
  const locationParts = siteConfig.address
    .split(",")
    .map((part) => part.trim());
  ...
  const galleryAlt =
    tab === "photos"
      ? `${previewCategory} at ${siteConfig.name} in ${location}`
      : `${amenityCategory} at ${siteConfig.name} in ${location}`;
      ...
      <div className="mx-auto max-w-[1920px]">
        ...
        <div className="mt-6 flex flex-col gap-4 font-[Plus_Jakarta_Sans] xl:flex-row xl:items-center xl:justify-between">
          ...
          <div className="flex gap-2">
            {availableBedroomTypes.map((type) => {
              const active = activeBedroomType === type;

              const label = type === "1bed" ? "1 Bedroom" : "2 Bedrooms";
              const price = bedroomPricing[type];
          ...
          <div className="overflow-hidden rounded-[20px] border border-[#ddd7cc] bg-[#fbfaf7] shadow-[0_2px_8px_rgba(26,29,27,0.04)] sm:rounded-[26px]">
            ...
            <div className="relative h-[260px] border-b border-[#ddd7cc] bg-white sm:h-[360px] lg:h-[500px]">
              {floorPlanImage ? (
                <img
                  src={floorPlanImage}
                  alt={`${currentPlan.title} floor plan at ${siteConfig.name}`}
            ...
            <div className="grid grid-cols-2 border-b border-[#ddd7cc] sm:grid-cols-4">
              {[
                ...
                ["Available", currentPlan.available ?? "Contact Leasing"],
            ...
            <div className="px-5 py-4 font-[Plus_Jakarta_Sans] sm:px-6 sm:py-5 md:px-7">
              <p className="mb-4 text-[11px] font-semibold uppercase tracking-[0.16em] text-[#50627a]">
                Plan Features
              </p>
              ...
              <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3 lg:gap-8">
                {[
                  ...
                  <div key={String(title)}>
                    <p className="mb-3 text-[12px] font-semibold uppercase tracking-[0.12em] text-[#1e3872]">
                      {title}
                    </p>
